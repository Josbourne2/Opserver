...
CREATE PROCEDURE [dbo].[sp_mon_Backup]
...
delete from dbo.Mon_Backups where backup_datum <GETDATE() -60
...
--print @sqlqry
		--exec (@sqlqry)
	select @sqlqry = 'insert into dbo.Mon_Backups(server,instance_id,dbid,type,duur,backup_size,backup_datum,database_name,backup_file)  '+ @sqlqry
	print @sqlqry
		exec (@sqlqry)
