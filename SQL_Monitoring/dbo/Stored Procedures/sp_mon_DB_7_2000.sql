...
CREATE PROCEDURE [dbo].[sp_mon_DB_7_2000]
--drop table #test
AS
BEGIN
...
--	20-12-2011	avdberg
--		uitbreiding sp_mon_db_2000, nu ook met SQL7 (m.n. linked server aanmaak)
-----------------------------------------------------------------------------------------
...
declare @cnt int,@error int,@ver nvarchar(50)
Declare @Linked_server varchar(500)

Declare @xTSQL1 nvarchar(4000)
Declare @xTSQL2 nvarchar(4000)
Declare @xTSQL3 nvarchar(4000)
Declare @xTSQL4 nvarchar(4000)
Declare @xsqlqry nvarchar(4000)
...
--2000 select @TSQL3 =  '''SELECT name, dbid, mode, status, status2, crdate, category, cmptlevel, filename,version,SUSER_SNAME(sid) from master.sys.sysdatabases''' 

select @xTSQL3 =  '''SELECT name, dbid, mode, status, status2, crdate, category, cmptlevel, filename,version,SUSER_SNAME(sid) from master..sysdatabases''' 
select @xTSQL3 = @xTSQL3 + ') a '
select @xTSQL3 = @xTSQL3 + 'full outer join (select instance_id as md_instance_id,id as md_id, [crdate] as md_createdate ,[dbname] as md_name,status as md_status,server,creator as md_creator from dbo.Mon_DB where deldate is null and instance_id = '
select @xTSQL4 = ') md on a.name COLLATE DATABASE_DEFAULT = md.md_name COLLATE DATABASE_DEFAULT '
select @xTSQL1 = ' from openquery('

select @TSQL1 = 'select * from openquery ('
select @TSQL2 = ''''' as servername, ''''' 
select @TSQL3 =  ''''' as instance_id, name, dbid, mode, status, status2, cast(crdate as smalldatetime) as crdate, category, cmptlevel, filename,version,SUSER_SNAME(sid) as creator from master..sysdatabases order by dbid'')' 
...
declare node_cursor Cursor For
	select  server,domein,id,versie from mon_instance where (versie = '2000'  or versie like '7%') and te_bewaken = '1' 
   order by id
open node_cursor
	Fetch Next from node_cursor
	into @srvr, @TLok,@instid,@ver
...
--linked server aanmaken
		if @ver like '7.%' or @ver like '6.%'
			BEGIN
				...
				if @error = 1
					print 'Aanmaken linked server < 2000 niet gelukt.'
			END
		else
			BEGIN
...
exec sp_executesql @sqlqry
	...
	--select @xTSQL5 = @xTSQL4 + 'where md.md_name is null or a.name is null or a.status <> md.md_status  or md_creator is null'
	--select @xTSQL6 = @xTSQL3 +'''' + cast(@instid as varchar(10)) + ''''+ @TSQL5
	--select @xsqlqry = @xtsql1 + @xtsql2 + @xTSQL6 --+ ') b '
	--print @xsqlqry
	--select @xsqlqry = 'insert into #test select * , ''' + @srvr1 + '''  as servername, ''' + cast(@instid as varchar(10)) + ''' as instance_id  '+@sqlqry --+ ' where name = b.name and server = ' +  @srvr1 + ''
	--print @xsqlqry
