...
CREATE PROCEDURE [dbo].[sp_mon_DB_files_2000_ev_old] 
...
declare @vnode nvarchar(50), @srvr nvarchar(50), @srvr1 nvarchar(50),@instid int,@ver varchar(50)
...
declare node_cursor Cursor For
	select node, instance,versie,id from mon_instance where controle = '1' and isnull(te_bewaken,1) =1  --and (versie  like '7.%' or versie  like '6.%' )-- and controle = '1' 
...
if exists (select srvid from master..sysservers where srvname = @srvr1) 
	or exists (select *,server_id from master.sys.servers where name = @srvr1) 
	BEGIN
...
--nieuwe files toevoegen
	insert into mon_db_files (instance_id,dbid,name,fileid,filename, filegroup, size_kb, maxsize_kb, growth, usage,date)
	select f.instance_id,f.dbid,f.name,f.fileid,f.filename, f.filegroup, f.size_kb, f.maxsize_kb, f.growth, f.usage,f.date from #files f
...
--bestaande entries  bijwerken
	update mon_db_files
	set filename=f.filename, filegroup=f.filegroup, size_kb=f.size_kb, maxsize_kb=f.maxsize_kb, growth=f.growth, date = getdate()
