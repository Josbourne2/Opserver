...
CREATE PROCEDURE [dbo].[sp_mon_Jobs_7_2000] 
--drop table #test
AS
BEGIN
...
-----------------------------------------------------------------------------------------
--select '306' as instance_id ,*  from openquery([BORPIS05], 'SELECT  sj.name as jobname,sj.job_id,sj.enabled as job_enabled,ss.schedule_id,ss.name as schedule_name,ss.enabled as schedule_enabled,SUSER_SNAME(sj.owner_sid) as owner ,sj.date_created,sj.date_modified 
...
-----------------------------------------------------------------------------------------
--SELECT  * --sj.name as jobname,sj.job_id,sj.enabled as job_enabled,ss.schedule_id,ss.name as schedule_name,ss.enabled as schedule_enabled,SUSER_SNAME(sj.owner_sid) as owner ,sj.date_created,sj.date_modified 
...
create table #instance (
    ...
    insert into #instance (node,instance,versie,id)
	select node, instance,versie,id from mon_instance where (versie like '7.%' or versie like '6.%' or versie like '2000%' ) and controle = '1'  
...
declare node_cursor Cursor For
	select node, instance,versie,id from #instance 
...
--SELECT  j.jobname,j.job_id,j.schedule_id,j.schedule_name,j.owner ,j.crdate,j.updatedate,lj.updatedate,lj.deldate
FROM #jobs j
inner join
dbo.mon_jobs lj on j.job_id =lj.job_id and j.instance_id = lj.instance_id and isnull(j.schedule_id,1234567) =isnull(lj.schedule_id,1234567)
...
update mon_jobs 
set deldate=GETDATE()
--SELECT  dbo.instname(lj.instance_id),lj.jobname,j.jobname,j.job_id,j.schedule_id,j.schedule_name,j.owner ,j.crdate,j.updatedate,lj.updatedate,lj.deldate
FROM mon_jobs lj
full outer join
...
where j.jobname is null and lj.crdate is not null
and  lj.instance_id in (select id from #instance)
