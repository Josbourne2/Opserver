...
CREATE PROCEDURE [dbo].[sp_mon_Logins_2005_2008] 
--drop table #test
AS
BEGIN
...
-- fouten uit script gehaald, mn de linked serverconnectie
...
Declare @inst nvarchar(64),@error int, @error_message nvarchar(max)
...
,ml_id int
,ml_createdate datetime,ml_updatedate datetime, ml_name varchar(100)
,server varchar(300),servername varchar(300)
,instance_id int
)
...
select @TSQL3 =  '''SELECT  [createdate],[updatedate] ,[name],status,accdate,dbname,language, [denylogin] ,[hasaccess]  ,[isntname],[isntgroup] ,[isntuser],[sysadmin],[securityadmin],[serveradmin],[setupadmin],[processadmin],[diskadmin],[dbcreator],[bulkadmin],[loginname] FROM [master].sys.[syslogins]''' 
...
declare node_cursor Cursor For
	select node, instance,domein,id from mon_instance 
	where (versie >= '2005' ) and controle = '1' and te_bewaken = 1
...
--linked server aanmaken
	exec dbo.sp_mon_CreateLinkedServer @server = @srvr1, @error = @error OUTPUT, @error_message = @error_message OUTPUT
if isnull(@error,0) =1 goto error
--print @inst
--Print @srvr
--print @srvr1
...
error:
	begin
	print 'Connectie naar '+ @srvr1 + ' lukte niet want: '+ ISNULL( @error_message,'')
	end
ok:
	begin 
	print 'ok'
