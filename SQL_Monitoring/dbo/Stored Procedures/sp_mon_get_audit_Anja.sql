...
CREATE PROCEDURE [dbo].[sp_mon_get_audit_Anja] @ServerName VARCHAR(255), @InstanceName VARCHAR(255)
...
BEGIN
	...
	FROM sys.fn_get_audit_file (''''L:\SQL_AUDITS\'+@InstanceName+'\*.sqlaudit'''',default,default) gaf
		INNER JOIN sys.dm_audit_class_type_map tm
			ON gaf.class_type = tm.class_type
		LEFT JOIN sys.dm_audit_actions aa
			ON gaf.action_id = aa.action_id
			AND aa.class_desc = tm.class_type_desc
	where   statement not like N''''RESTORE VERIFYONLY%''''
		AND statement not like N''''RESTORE LABELONLY%''''
		AND statement not like N''''UPDATE STATISTICS%''''	
			'') AS a;'
	print @sql;
