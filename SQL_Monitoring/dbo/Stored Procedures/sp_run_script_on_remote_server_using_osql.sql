...
CREATE PROCEDURE [dbo].[sp_run_script_on_remote_server_using_osql]
   @node		varchar(30)	  
...
set @TSQL = 'DECLARE inst_csr CURSOR
      FOR
select node from logcontrole.dbo.C_sqlnodes where controle =1 '
...
set @TSQL = @TSQL + ' order by node'
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @script = 'CALL OSQL -S"' + @instance + '" -E -Q"' + @command + '" >> %_LOG_%'
         END
         ELSE
         BEGIN
            --Filenaam: gebruik -i-optie
            --Default database is master
            SET @script = 'CALL OSQL -S"' + @instance + '" -E -d"master" -i"' + @filenaam + '" -n >> %_LOG_%'
         END
